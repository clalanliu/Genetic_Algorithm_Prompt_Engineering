...
def init_run(population: Population, model: Client, num_evals: int):
    ...
    assert len(
        results) == population.size, "size of google response to population is mismatched"
...
def _evaluate_fitness(population: Population, model: Client, num_evals: int) -> Population:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(examples)) as executor:
        future_to_fit = {executor.submit(
            model.batch_generate, example_batch,  temperature=0): example_batch for example_batch in examples}
        for future in concurrent.futures.as_completed(future_to_fit):
            # Get the prompt corresponding to this future
            example_batch = future_to_fit[future]
            try:
                data = future.result()
                results.append(data)
    ...
    for unit_index, fitness_results in enumerate(results):
        for i, x in enumerate(fitness_results):
            valid = re.search(gsm.gsm_extract_answer(
                batch[i]['answer']), x[0].text)
            if valid:
                # 0.25 = 1 / 4 examples
